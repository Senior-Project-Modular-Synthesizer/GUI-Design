...
lv_obj_t * header_create(lv_obj_t * parent, const char * title)
{
    ...
    if (!style_inited) {
        lv_style_init(&header);
        lv_style_set_bg_color(&header, lv_color_hex(0x2d2d2d));
        lv_style_set_border_width(&header, 2);
        lv_style_set_border_side(&header, LV_BORDER_SIDE_BOTTOM);
        lv_style_set_border_color(&header, lv_color_hex(0xa2a2a2));
        lv_style_set_radius(&header, 0);
        lv_style_set_width(&header, lv_pct(100));
        lv_style_set_height(&header, 32);
        lv_style_set_pad_hor(&header, 8);
        lv_style_set_layout(&header, LV_LAYOUT_FLEX);
        lv_style_set_flex_cross_place(&header, LV_FLEX_ALIGN_CENTER);
        lv_style_set_flex_track_place(&header, LV_FLEX_ALIGN_CENTER);
        lv_style_set_flex_flow(&header, LV_FLEX_FLOW_ROW);
        lv_style_set_text_color(&header, lv_color_hex(0xffffff));
    ...
    lv_obj_add_style(lv_obj_0, &header, 0);
